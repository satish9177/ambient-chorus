...
session-scoped fixture builds all six real ZIPs once, into a session temp directory, and the
...
def built_lambda_artifacts(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Path]:
    """Build all six Lambda ZIPs into a session temp directory; yield that directory.
