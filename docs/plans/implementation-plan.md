...
**Tasks:** approve/reject exact `proposal_hash`/`view_hash`/`preview_hash`; expiration; DRAFT→APPROVED with the approval-dependent fields becoming present; the explicit reject path that atomically invalidates the current action pointer, fails the `DRAFT` execution, and returns the case to `READY_FOR_ACTION`; async send operation; approval consume + SENDING CAS; final fence; server-side destination; render and `rendered_message_hash` compared against the approved `preview_hash`; one SES call/tag; SENT/FAILED/SEND_UNKNOWN; fence release; recovery/reconciliation; action case projection.

The fence checks the case's **state** and its `authorization_version`, and must never require the current `CommunityCase.version` to equal the proposal's recorded `case_version` — lifecycle progression moved that number on purpose, and requiring it would fail every first send ([ADR-020](../adr/ADR-020-case-authorization-version.md) § 7).
