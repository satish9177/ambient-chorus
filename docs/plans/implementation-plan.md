...
An independent review of the implementation reproduced six defects that the three ADRs permitted but did not require, and the repair pass amended each ADR where the fix changed a frozen contract rather than only the code:

- **The claim compare-and-swap alone did not prevent a duplicate send.** A worker whose claim transaction had an *ambiguous* outcome resolved it against a commit proof keyed on the execution, read a proof another worker had written, and concluded it held a claim it did not. `ActionExecution` gains `claim_owner_hash`, minted per attempt and required from `SENDING`; `action-execution/v2` becomes `/v3` ([ADR-025](../adr/ADR-025-one-deliberate-ses-attempt.md) § 1).
- **Send-time authorization was validated before the fence was taken**, leaving a window in which a contributor's revocation could commit unseen by both halves. The compiler now acquires the fence first and revalidates from inside it, releasing on any denial (§ 4).
- **The clock was sampled before two awaited registry lookups.** A second sample now happens after every resolution, immediately before the SES call (§ 3, step 9b).
- **Worker recovery stranded an unfinished attempt and swallowed a lost case projection.** A `RUNNING` operation over an `APPROVED` execution resumes, and an operation does not report success while it still owes the case its `ACTIONED` edge.
- **A committed approval whose completion write was lost answered `EXECUTION_NOT_DRAFT`.** An `IN_PROGRESS` receipt is now resolved by reading the transaction's own commit proof ([ADR-023](../adr/ADR-023-approval-binding-and-immutability.md) § 6b), and the approval regenerates the preview so `from_identity_id` and `template_version` — bound only through `preview_hash` — are actually checked (§ 5, check 7b).
- **The deployed sender composition built a `CoreRepository`** it is denied. The authorization boundary is now `chorus.ports.send_authorization.SendAuthorizationPort`, with an in-process implementation for local runs and a compiler-invocation adapter for the deployed one ([ADR-024](../adr/ADR-024-execution-partition-and-sender-boundary.md) § 3).

Two contract corrections came with them. `failure_detail_safe` becomes `ABSENT` at `SEND_UNKNOWN` — it was `OPTIONAL` there and `ABSENT` at `SENT`, which monotonic presence makes unreconcilable. And ADR-025 § 10's "operator route" becomes the **trusted configuration-set event path**: a message identifier is a value only SES can produce, so the entry point takes an event envelope and has no parameter for a caller-asserted identifier, tag, or configuration set. Phase 8 owns that decode-and-verify boundary; the event destination and its transport are Phase 11.
...
**One Phase-8 contract is amended by Phase 9.** The action case projection goes from **four** participants to **five**, adding the immutable outbound message locator ([ADR-026](../adr/ADR-026-inbound-reply-trust-and-correlation.md) § 3). It is additive — no existing participant, field, or condition changes — and it is there rather than in the send-outcome transaction because the projection is run by the application worker while the send outcome is run by the sender. Without it there is no index from an SES message identifier back to an execution, and therefore no way for any reply to correlate to anything.
...
**Dependencies:** the action sent path and its case projection, the Phase-5 Investigator, the Phase-7 grounding grammar, and the Phase-8 attester/verifier pattern.

**Files/modules:** `chorus.ports.inbound_mail`, `chorus.ports.scheduler`, `chorus.application.services.inbound_mail` and `commitment_validation`, `chorus.contracts.commitment`, the `ingest_external_reply`, `extract_commitment_operation`, `apply_commitment`, `create_due_schedule`, `record_commitment_due`, and `verify_commitment` commands, `chorus.infrastructure.scheduler` and `chorus.infrastructure.local.inbound_mail`, `functions/commitment_watcher`, the inbound entry point of the worker artifact, `infra/cdk/stacks/watcher.py` and its IAM assertions, the verification API, the demo clock, DLQ/alarms.

**Tasks:** the inbound trust boundary and its transport-authenticator port; `In-Reply-To` correlation against the immutable outbound message locator; deterministic body extraction, quoted-outbound removal, and the immutable inbound artifact; the `EXTRACT_COMMITMENT` operation and its span-cited output contract; the nine deterministic grounding checks; commitment state including the two human-guard corrections; deterministic schedule name, client token, and configuration; response-loss reconciliation; due-event derivation and idempotency; the watcher `PENDING→DUE` transition; the fulfilled/missed human guard; case resolution and readiness; logical/actual clock mapping.

Three ADRs are accepted at the Phase-9 freeze gate, each closing a place where implementation would otherwise have invented security-sensitive semantics.

[ADR-026](../adr/ADR-026-inbound-reply-trust-and-correlation.md) removes the caller-supplied reply. `POST /demo/external-replies` accepted `{case_id, action_id, from_destination_id, subject, text}` — every field a reply's provenance depends on, typed by whoever holds the demo token, which is T18 with the attacker handed the pen and the identical shape [ADR-025](../adr/ADR-025-one-deliberate-ses-attempt.md) § 10 had already removed one boundary over. It replaces it with the same two-half attester/verifier boundary over a transport-authenticator port, adds the correlation channel that did not exist — the `chorus_execution` tag never reaches a recipient and `reply_to_address` carries no per-execution component — and introduces the **immutable authenticated external-source binding** [ADR-015](../adr/ADR-015-evidence-status-and-verification.md) § Revisit condition asked for: destination ID, registry version, and routing token as they stood at ingestion, recorded durably on the stored evidence. `EvidenceItem.submitted_by_contributor_id` becomes nullable rather than management being written into private storage as a resident contributor — the field, not the truth, is what changes. Adding this binding does **not** make the source eligible to grant `EvidenceStatus.VERIFIED`; the allowed verification source set stays empty, and adding a member is a separate explicit ADR that must state where the permission stops.

[ADR-027](../adr/ADR-027-commitment-extraction-grounding-and-authority.md) gives the extraction an algorithm. "Due time is explicitly supported by the cited reply" was unenforceable while the only citation was `source_evidence_id` — *which reply*, never *which words* — the same gap [ADR-021](../adr/ADR-021-action-grounding-and-caveats.md) closed on the outbound side. Proposals now cite character spans into the stored text, the obligor is asserted by the correlation rather than extracted, the deadline is a cited ISO date and the model's own `due_at` is never read, and a frozen conditional-token list keeps "we'll look into it" from becoming a promise. It also corrects two guards that were weaker than the documents they implement: `DUE→FULFILLED`, `DUE→MISSED`, and `VERIFYING→READY_FOR_ACTION` all now require a human actor, leaving `PENDING→DUE` as the state machine's one system edge.

[ADR-028](../adr/ADR-028-deadline-watcher-and-scheduler-boundary.md) removes the word "signed" from a due event nobody had built a verifier for, replacing it with the stronger check already present — every field re-verified against the strongly-loaded commitment — and resolves the entity that required what the transaction was said to attach afterwards: `scheduler_name`, `schedule_generation`, and `due_event_id` are derived at creation, and `PENDING_SCHEDULE` is an operational projection rather than the `CommitmentStatus` it was never a member of.

**Phase 9 owns the inbound boundary; Phase 11 owes it exactly one object** — an `InboundMailTransportAuthenticator` over the SES receipt transport it builds — in the same static-now split the compiler, the three runtimes, and the sender already use. With no authenticator the attester refuses every delivery, which is the correct outcome and not a gap.

**Tests:** the boundary with no authenticator wired; a hand-built attestation; a forged `In-Reply-To` from a foreign sender; a reply to a `SEND_UNKNOWN` execution and to a terminal case; duplicate delivery; quoted-outbound-only text; oversized, attachment-bearing, and header-truncated replies; malicious reply and date; uncited, ungrounded, conditional, wrong-obligor, and duplicate commitments; a model-invented deadline; a lost apply commit; schedule explicit and lost response; duplicate and stale generation; early firing; DLQ configuration; due twice; unauthorized verifier; `ACTIONED` direct-to-`RESOLVED` illegal; the fulfilled/missed human guards; the demo clock path; and the participant-count assertions for all six new transaction shapes.

**Exit criteria:** against the local composition and DynamoDB Local, an attested fixture reply produces exactly one commitment, exactly one schedule request under the deterministic name and client token, exactly one `PENDING→DUE` transition across duplicate and early invocations, exactly one verification request, and the two human outcomes — `FULFILLED` alone resolving and `MISSED` returning the case to `READY_FOR_ACTION` with the action pointer invalidated. **The live reply, the live schedule, and their smoke tests are Phase 11**, alongside every other live AWS resource; Phase 9 owns the watcher artifact, its role, its log group, the schedule group and DLQ definitions, and deploys none of them.
...
**Do not implement:** recurring calendars, a conversational reminder agent, automatic resolution, Step Functions, a second commitment class, a reschedule or deadline-change verb, a commitment cancellation route, a `DISPUTED` status, an `ACTIONED→READY_FOR_ACTION` command, a new allowed verification source, HTML body parsing, attachment acceptance, or any live AWS deployment.
