...
| forwarded evidence | collapse parent/forward chain to earliest root | deterministic root traversal; cycles reject | forward relation recorded | marked forwarded; no independence gain |
...
| contradiction | store `CONTRADICTED`/contradiction with citations; do not erase evidence | assessment replay-safe | investigating or ready only if nonfatal and caveated | visible contradiction/caveat |
