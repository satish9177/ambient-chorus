...
| deployment configuration moved between proposal and approval | approval-time exact-equality check refuses before anything is written | not retryable as-is | nothing written; the `DRAFT` stands | 409 stale authorization; the human may still reject |
| a stale proposal is **rejected** | rejection re-checks only scope, pointer, and execution version | replay-safe under the same key | pointer `INVALIDATED`; execution `FAILED`; case `READY_FOR_ACTION` if readiness remains | `action.rejected`; a stale proposal is never unrejectable |
| a proposal is rejected while readiness has been lost | the pointer and execution clear; **no case edge is taken** | none | case stays `ACTION_PROPOSED`; the case participant is a `ConditionCheck` | readiness reconciliation, not the rejection, owns `ACTION_PROPOSED→INVESTIGATING` |
| human withdraws an approval while the sender is claiming | both are conditional writes to `APPROVED@v`; exactly one commits | neither retries | one of `FAILED/APPROVAL_WITHDRAWN` or `SENDING` | 409 for the loser; no window in which both won |
| invalidation attempted against `SENDING`, `SENT`, or `SEND_UNKNOWN` | refused | none | unchanged | 409; a send in flight cannot be taken back and a sent message cannot be recalled |
| regenerated preview digest differs from the approved `preview_hash` | definite pre-send failure **before** the claim and before any SES call | not retryable as-is | execution `FAILED/STALE_AUTHORIZATION`; nothing consumed | recompile, re-propose, reapprove |
| an SES exception class nobody enumerated | classified `SEND_UNKNOWN` by default | never resend | `SEND_UNKNOWN` | the safe side is the default, not the remembered case |
| claim transaction outcome lost, and another worker had claimed | the durable row names another attempt's `claim_owner_hash`; **no SES call**, nothing written | none; the owner finishes its own attempt | unchanged | `execution.claim.not_owned` at WARNING; the caller is answered from the durable row |
| claim transaction outcome lost, and nothing committed | the row stands at `APPROVED`; a redelivery resumes the same claim | ordinary retry | `APPROVED` | no alarm; the frozen safe-retry branch |
| mandate revoked between send-time validation and fence acquisition | impossible: the fence is taken first and the revalidation runs inside it | recompile, re-propose, reapprove | `FAILED / STALE_AUTHORIZATION` | **no SES call** |
| destination or sending-identity resolution crosses the fence expiry | the clock re-read immediately before the call refuses | recompile, re-propose, reapprove | `FAILED / FENCE_EXPIRED` | **no SES call** |
| execution `SENT` and the case projection to `ACTIONED` failed | the operation stays recoverable, and a replay repairs the projection | automatic on redelivery | `SENT`; case reaches `ACTIONED` | **no second SES call**; `PROJECTION_DEFERRED` until it lands |
| approval transaction committed and its completion write was lost | an identical retry reads the transaction's commit proof and replays the recorded decision | none | `APPROVED` | no second approval and no second `DRAFT` compare-and-swap |
| sending identity or template version moved between proposal and approval | the approval regenerates the preview and refuses | recompile and re-propose | execution stays `DRAFT` | 409 `PREVIEW_BINDING_MOVED` |
| reconciliation event carries a message ID disagreeing with one already recorded | refused; monotonic presence forbids the rewrite | never | unchanged | `INTEGRITY_ERROR`; a forged message ID is at worst a rejected reconciliation |
| concurrent approval/double approve | the execution's `DRAFT@1` compare-and-swap admits exactly one decision | exact key returns winner; other conflicts | one approval/one execution approved | 409 for conflicting decision |
...
| S3 derivative written but DB compile fails | the object is an unreferenced orphan and confers no authority; nothing references it and no current view ever may | a retry re-derives identical bytes and writes the identical content-addressed key, so it is the same write; lifecycle cleans an abandoned orphan | no view, no history locator, no pointer change | persistence failure, no export |
| S3 derivative PUT outcome unknown | `HEAD` the exact content-addressed key: present with the expected hash, byte length, and allowlisted metadata means present; absent means repeat the identical PUT; present and inconsistent is `INTEGRITY_ERROR` | never create a second key because an outcome was unknown | one object per distinct derivative | dependency event; no partial export state |
| compile denied | no view, no history locator, no safe-evidence reference, and **no change to the current pointer** | the denial's audit event, audit projection, and completed idempotency record commit atomically, so a redelivery replays the answer rather than re-running the compile | current view, if any, remains valid and current | `compile.denied` with reason codes; 422 `POLICY_DENIED` |
