...
17. **Contributor/evidence independence:** contributor count uses unique contributor IDs; corroboration uses unique contributor plus collapsed `EvidenceRoot`. Forwarded/duplicate roots cannot increase evidence sufficiency. `CORROBORATION_MIN=2` is rechecked but never substituted for privacy count.
...
| `p1.contradiction.safe.v1` | management statement + contradictory incident facts | neutral contradiction statement | cites safe facts; no private quote unless explicitly safe |
