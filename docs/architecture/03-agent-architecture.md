...
The Action Agent **never reads `InvestigationAssessment`**. Contradiction materiality is deliberately absent from the view: Phase 5 is the sole authority that `MEDIUM` and `HIGH` contradictions block readiness, so any contradiction reaching a current view is `LOW`, and the caveat obligation [ADR-015](../adr/ADR-015-evidence-status-and-verification.md) § 7 left to this phase is discharged from `ShareableFact.evidence_status` alone.
