...
It is a **field-for-field mirror** of the view, not a second lossy projection. The mirror exists only because [13-repository-and-coding-standards.md](13-repository-and-coding-standards.md) freezes `contracts.action` as self-contained public-safe primitives, so it may import neither `chorus.privacy` nor `chorus.domain` and must restate the view's shape and enums; a parity test asserts the field sets never drift, exactly as the stored-view mirror already does. The application may strongly read the Core case to enforce freshness, and **that private read is never appended to the payload**.
...
- `view_id: UUID`, `view_hash: Sha256Digest`, `case_id: UUID`, `case_version: int`, `authorization_version: int`;
- `subject: str` (1–120 characters, no newline);
- `claims: tuple[ActionClaimDraft, ...]` (1–12), each `{claim_id, text 1–500, export_fact_ids 1–10}`;
- `request: {requested_action: str 1–500, requested_deadline: UTC datetime?, request_fact_ids 1–10}`;
- `caveats: tuple[ActionCaveatDraft, ...]` (0–8), each `{caveat_id, text 1–500, export_fact_ids 1–10}`;
- `tone: NEUTRAL | COLLABORATIVE | FIRM`.

**Every model-authored substantive field is citation-bound.** Claims, the request, and caveats each carry 1–10 citations and **never zero**. `request.requested_action` remains normative preference rather than a factual claim — it asserts nothing — and it still cites the externally safe facts that justify asking, because a request without a reason is a request the recipient cannot evaluate and because the alternative is a factual-premise classifier that only an unspecified parser or a second model could implement ([ADR-021](../adr/ADR-021-action-grounding-and-caveats.md) § 1).

`claim_id` and `caveat_id` are model-local within one output and are deliberately UUID-shaped: they are persisted, they name nothing outside their own proposal, and they grant nothing. The identifier-shape guard that refuses a UUID-shaped Monitor `client_ref` does not apply to them.

In the four model-authored textual fields the deterministic validator rejects newline/header injection, markup, URLs, `mailto:`, email and telephone patterns, quotation, non-ISO date constructs, identifier shapes, and unsupported numbers, dates and proper names; across the whole proposal it rejects non-view IDs, duplicate claim or caveat IDs, duplicate normalized text, empty citation sets, an uncaveated contradicted fact the proposal relies on, and wrong or stale view identity. The prose rules apply to prose: the typed contract identifiers a proposal must carry are validated for view and case membership, not scanned as text. The exact normalization, rejected constructs, token grammar, and support comparison are frozen in [ADR-021](../adr/ADR-021-action-grounding-and-caveats.md); a false positive rejects and requires a re-proposal, never a bypass and never a second model's opinion. The validator then assigns `action_id` and `execution_id`, canonicalizes, hashes, renders the preview, and persists. No free-form body supplied by the model is accepted.

The Action Agent **never reads `InvestigationAssessment`**. Contradiction materiality is deliberately absent from the view: the investigation apply is the sole authority that `MEDIUM` and `HIGH` contradictions block readiness, so any contradiction reaching a current view is `LOW`, and the caveat obligation [ADR-015](../adr/ADR-015-evidence-status-and-verification.md) § 7 assigns to the Action proposal validator is discharged from `ShareableFact.evidence_status` alone.
...
- Prompt IDs are `monitor/v3`, `investigator/v1`, and `action/v1`; prompt text is version-controlled next to each runtime. `action/v1` names the first reviewed Action artifact — the prompt text together with the structured-output model frozen by [ADR-021](../adr/ADR-021-action-grounding-and-caveats.md). No Action runtime artifact has ever existed, so there is no earlier version to bump away from. The prompt states that view text is untrusted data, that every claim, request, and caveat must cite `export_fact_id` values, that `YYYY-MM-DD` is the only permitted date form, and that quotation, invented quantities, invented names, HTML, a complete email body, and tool use are all outside what it may return. A pinned prompt version names the whole reviewed artifact — the prompt text *and* the structured-output model the runtime asks for — because the runtime passes both in one call. The Monitor moved to `v2` with [ADR-012](../adr/ADR-012-candidate-grouping-invariant.md), which put the candidate-grouping invariant into the instructions rather than leaving it as a validator rule the model was never told about, and to `v3` with [ADR-014](../adr/ADR-014-monitor-proposes-no-disclosure-terms.md), which removed `mandate_suggestions[]` from the output schema. In both cases the version moves so a runtime serving the old artifact is refused once, by version, rather than failing every batch it answers.
