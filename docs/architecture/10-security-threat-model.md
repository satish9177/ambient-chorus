...
V1 assumes AWS account/Region controls and Bedrock/AgentCore/SES services perform as documented; the single-presenter access token is a known non-production identity compromise. **Inbound email authenticity is in scope and is delegated to the transport**: the boundary that consumes it is built and tested in Phase 9, and the one `InboundMailTransportAuthenticator` that satisfies it is Phase 11's ([ADR-026](../adr/ADR-026-inbound-reply-trust-and-correlation.md)). Until that object exists the attester refuses every delivery, which is the correct outcome rather than a gap.
...
| T18 | malicious email creates commitment/marks resolved | workflow manipulation | a reply is not a reply until an authenticated transport delivered it, the SES receipt verdicts all passed, and it correlated to exactly one `SENT` execution; cited-span term/date validation, range/cap, the agent cannot persist, and the affected contributor alone verifies fulfillment ([ADR-026](../adr/ADR-026-inbound-reply-trust-and-correlation.md), [ADR-027](../adr/ADR-027-commitment-extraction-grounding-and-authority.md)) | closed refusal codes across transport, decode, correlation, and body, each audited with no content; residual is that Phase 11 owns the authenticator |
| T19 | scheduler replay/old generation/early firing changes state | false follow-up | the event is re-verified against the strongly-loaded commitment; a mismatched namespace, case, generation, or due event ID, a status other than `PENDING`, or `now < due_at` is a success no-op that changes nothing and reschedules nothing; the verification-request item is create-only ([ADR-028](../adr/ADR-028-deadline-watcher-and-scheduler-boundary.md) § 3) | replay/early/stale-generation events and metrics |
...
| T36 | a forged inbound reply, attributed to the approved destination, manufactures a management promise on any case | a fabricated commitment, a false schedule, and a case that looks answered | the caller-supplied reply body is removed from the API entirely — the demo route takes a fixture selector; the attester authenticates the transport *before* decoding, requires SPF/DKIM/DMARC `PASS`, requires an `In-Reply-To` resolving one immutable outbound message locator, and requires the DMARC-verified sender digest to equal the correlated destination's; a `SEND_UNKNOWN` execution has no locator and no reply can attach to it | forged-reply, foreign-transport, wrong-sender, wrong-recipient, and replay tests; residual is that Phase 11 supplies the only authenticator |
| T37 | a reply quotes our own outbound message and a model grounds a commitment against text we wrote | a deadline we requested returns as a promise management never made | the outbound body is regenerated from the immutable proposal, view, and template version, and every normalized reply line equal to one of its lines — plus every line beginning with `>` — is deleted before extraction | a reply consisting only of our own quoted message yields no grounded commitment |
| T38 | prompt injection in an inbound reply instructs the extractor to resolve the case or emit policy | model-driven state change | the extraction contract has no status, case-state, destination, or verification-method field; the apply command holds no SES, compiler, or scheduler port; the model's own `due_at` is measured and never read; obligor comes from the correlation and the deadline from a cited ISO date | injection corpus; a run whose output tries to assert anything else fails `extra=forbid` |
| T39 | a model marks a commitment satisfied or a case resolved | the system reports a fixed elevator that was never fixed | `FULFILLED` and `MISSED` are human-only edges guarded in `transition_commitment`; `VERIFYING→RESOLVED` and `VERIFYING→READY_FOR_ACTION` require `actor_is_human`; the allowed verification source set of [ADR-015](../adr/ADR-015-evidence-status-and-verification.md) stays empty and `VERIFIED` stays unreachable | legal/illegal transition table; `VERIFIED` expected **0** times in V1 |
| T40 | a watcher invocation names a commitment in another namespace, case, or generation | cross-case state change from a queue message | the event is not trusted; namespace, case, generation, due event ID, and due time are all re-verified against the strongly-loaded row, and any mismatch is a success no-op that changes nothing and reschedules nothing | stale-generation, foreign-case, early-fire, and duplicate-delivery tests |
