...
Fields: `approval_id`, `namespace`, `community_id`, `case_id`, `action_id`, `execution_id`, `proposal_hash`, `view_hash`, `authorization_version`, `approver_id_hash`, `approver_assurance`, `decision: APPROVED|REJECTED`, `approved_at`, `expires_at`, `approval_hash`, `request_key_hash`, `version`, timestamps. The schema version is `approval/v2`.

**An approval is fully immutable.** It is written once and never updated, and `consumed_at` does not exist ([ADR-023](../adr/ADR-023-approval-binding-and-immutability.md) § 1). The execution reaching `SENDING` *is* the consumption — one-time by compare-and-swap on a row one principal writes — so recording it a second time here would have put a mutable field inside an authorization digest and would have required its writer to hold a write grant on the immutable proposal's own partition. `hash_approval` therefore omits only `{approval_hash, version, created_at, updated_at}`, and recomputation is meaningful at any later instant.

**One decision per proposal** is enforced by the execution's row version, not by a condition on this item: every decision moves the `DRAFT` execution out of `DRAFT` under `expected_version`, so exactly one of any number of concurrent approvals or rejections commits.

`expires_at` is `min(approved_at + 15 minutes, view.expires_at)`. `authorization_version` is the epoch current when the human decided and is **provenance only** — send-time authority is re-derived from live state by the fence, never read from here. `approver_id_hash` with `approver_assurance = DEMO_SHARED_TOKEN` records that somebody holding the demo access token asserted the approver persona; it is single-presenter demo access control and **not** authentication of a person. Approver identity is operationally sensitive and not sent externally.

### ActionExecution

Purpose: durable send attempt state and ambiguity record.

Fields: `execution_id`, `action_id`, `case_id`, `approval_id?`, `proposal_hash`, `view_hash`, `idempotency_key?`, `state`, `attempt_number: literal 1` in V1, `claim_owner_hash?`, `rendered_message_hash?`, `ses_request_token_hash?`, `ses_message_id?`, `started_at?`, `finished_at?`, `failure_code?`, `failure_detail_safe?`, `reconciled_at?`, `version`, timestamps. Rendered body and recipient are not stored here; the exact rendered body is reconstructible from immutable proposal/view/template version and its hash. The schema version is `action-execution/v3`.

**Two principals write this record, in disjoint state ranges.** The application creates the `DRAFT` and takes `DRAFT→APPROVED`, `DRAFT→FAILED`, and `APPROVED→FAILED` (withdrawal); the sender takes `APPROVED→SENDING` and the three terminal send outcomes. The separation is enforced by `ACTION_EXECUTION_EDGES` plus the exact-version condition on every write, not by IAM — `dynamodb:LeadingKeys` cannot distinguish two writers of one item type — and the single overlapping state, `APPROVED`, is the withdrawal race the compare-and-swap resolves with exactly one winner ([ADR-024](../adr/ADR-024-execution-partition-and-sender-boundary.md) § 4). The record lives in its own Shareable partition `NS#n#EXECUTION#a` so that the sender's write grant reaches it and cannot reach the proposal or the approval.

**Which fields are present is a function of `state`, and presence is monotonic**: a field that has been set is never unset or rewritten, and the entity refuses a transition that would clear one. The normative table is [ADR-022](../adr/ADR-022-action-draft-preview-and-transaction.md) § 1. In outline: a `DRAFT` created by the proposal apply carries no `approval_id`, no `idempotency_key`, no `rendered_message_hash`, and no `ses_request_token_hash`, because none of them exists before a human has approved anything and the send idempotency key is defined over the approval; `approval_id` and `idempotency_key` become required at `APPROVED`; the rendered and SES token hashes become required at `SENDING`. `FAILED` may legitimately carry none of them, because `DRAFT→FAILED` and `APPROVED→FAILED/STALE_AUTHORIZATION` both terminate before anything was rendered or sent — requiring a rendered hash there would put a fabricated digest on the record of a message that was never built. `claim_owner_hash` and the rendered and SES token hashes become required at `SENDING`. `failure_detail_safe` is `OPTIONAL` at `FAILED` and `ABSENT` everywhere else --- `SEND_UNKNOWN` included, because presence is monotonic and `SEND_UNKNOWN → SENT` has it absent, so a quarantined row carrying one could never be reconciled ([ADR-025](../adr/ADR-025-one-deliberate-ses-attempt.md) § Consequences).

`claim_owner_hash` is the one field on this entity that is **not** derivable from durable state, and that is its whole purpose: it is minted per send attempt and written in the same conditional write that moves `APPROVED → SENDING`, so a durable `SENDING` row answers *which attempt* owns the claim rather than merely that the execution was claimed. A commit proof keyed on the execution cannot answer the first question, and only the first question authorizes a sender to call SES.
