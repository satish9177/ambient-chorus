...
Its input is the case identifier, the safe destination display label, and the **single** inbound artifact's normalized `extracted_text`, delimited as untrusted data. Not the case, not other evidence, not facts, not mandates, not contributor data. The model that reads a stranger's email is given nothing else to leak.

> **Amendment (Phase 11, batch 3).** `CommitmentExtractionInput` gains `destination_display_label`. Live commitment extraction receives the already-safe configured destination display label **solely so `candidate.obligor` can satisfy the existing deterministic correspondent-binding check** (check 4 in § 3). The frozen demo reply — "we will restore elevator b to service by 2030-01-14." — contains no organization name, so a model given only the reply could satisfy check 4 only by accident and live extraction would reject correct answers. The label is non-secret by design ([the Phase 11 deployment contract](../plans/phase-11-deployment-contract.md) § 13 classes it a safe environment variable beside the registry version and the routing token); it names no mailbox and carries no address, token, or registry record.
>
> **Validation authority is unchanged.** Check 4 still compares the model's restatement against the value deterministic code holds, so the model can only agree with a fact the correlation already established or be rejected. `obligor_span` must still cite the reply's own words for the party who will act, so the proposal stays checkable against the reply as well as against who wrote it. The label is rendered outside the untrusted-data fence, and it is absorbed into `extraction_input_hash`, so an extraction produced under one correspondent is not recovery proof for another. No other Phase 9 semantic changes.
...
4. **Obligor is asserted, not extracted.** `normalize(obligor)` must equal the safe `display_label` of the destination the correlated execution sent to — a value that comes from the non-secret safe destination configuration and the binding [ADR-026](ADR-026-inbound-reply-trust-and-correlation.md) § 5 recorded, never from the reply and never from the model. → `COMMITMENT_OBLIGOR_MISMATCH`. The "wrong responsible party" failure is closed structurally: the model's value is only ever checked for agreement with a fact correlation already established. Per the § 1 amendment the model is now *shown* that label so it can restate it; that changes what an honest answer can look like and changes nothing about who decides — the comparison is still against the configured value, and a model that supplies a different one is still rejected here.
