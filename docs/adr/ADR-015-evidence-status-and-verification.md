...
[03-agent-architecture.md](../architecture/03-agent-architecture.md) says deterministic validation "forbids a `VERIFIED` status without an allowed verification source", and the Phase-5 entry of the implementation plan (since retired) lists "evidence status rules including verified-source rule" among its tasks. A repository-wide search returns those two sentences and the enum member. Nothing defines an allowed verification source, nothing defines the other four statuses, and nothing says whether "forbids" means reject or downgrade.
