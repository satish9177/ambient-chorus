...
the payload bound. The application then proves the answer is about the case that was sent.
...
answer; the application refuses any result that names a different one.

Nothing is logged. Not the payload, not the answer, not a truncated preview of either: the
payload is a whole private case by construction, and a runtime log group is not a private
evidence store.
"""
...
class InvestigatorRunner(Protocol):
...
def parse_invocation(raw: bytes) -> AgentInputEnvelope[InvestigationInput]:
    ...
    try:
        invocation = AgentInputEnvelope[InvestigationInput].model_validate_json(raw)
    except ValidationError as error:
        raise RuntimeContractError("payload is not a valid Investigator invocation") from error
    if invocation.agent_name is not AgentName.INVESTIGATOR:
...
async def handle(
    raw: bytes,
    *,
    runner: InvestigatorRunner | None = None,
    ...
    try:
        async with asyncio.timeout(budget_seconds):
            output: InvestigationAssessmentDraft = await active.run(invocation.payload, fence=fence)
